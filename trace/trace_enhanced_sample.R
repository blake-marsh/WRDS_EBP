...
#---------------------------------------
df = readRDS("./data/trace_enhanced_with_fisd_characteristics.rds")
...
#-------------------------------------------
# Keep trades that have fisd Characteristics
#-------------------------------------------
df = df[!is.na(fisd),]

fisd_count = df[,list(trades = .N,
                     CUSIPs = length(unique(cusip_id)),
                     banks = length(unique(id_rssd)))]
fisd_count[,step := "fisd Characteristics"]
...
#----------------------------
colnames(df)
df = df[which(!is.na(coupon_type) & coupon_type %in% c('F', 'V', 'STEP CPN')),]
head(df, 5)
...
#----------------------------------------------
df = df[which(!is.na(offering_date) & !is.na(maturity_date)),]
...
df[,maturity_yrs_orig := as.numeric(maturity_date - offering_date)/365]
df[,maturity_yrs_remain := as.numeric(maturity_date - trd_exctn_dt)/365]
...
## build table
count_table = rbindlist(list(raw_count,nontrade_days_count, fisd_count,
...
# Export the dataset
#--------------------
saveRDS(df, "/scratch/frbkc/trace_enhanced_sample.rds")
write.table(df, "/scratch/frbkc/trace_enhanced_sample.psv", row.names=F, sep="|", na = ".")
